...
#多进程给main方法添加一个paage，用以表示列表页的页码。
def main(page):
    #获取列表页的html
...
if __name__ == '__main__':
    """
    因为爬取10页的列表页互不干扰，因此我们可以一页开一个进程来爬取。而且这10个列表页页码正好可以提前构造成一个列表，所以我们可以选用
    多进程里面的进程池Pool来实现这个过程
    """
    ...
    #最后调用map方法，其中第一个参数就是需要被调用的参数(方法名)，第二个参数就是pages，即需要遍历的页码
    #这样就会依次遍历pages中的内容，把1-10这10个页码分别传递给main方法，并把每次调用分别变成一个进程，加入进程池中，进程池
    #会根据当前运行环境来决定运行多少个进程。
    pool.map(main, pages)
    pool.close()
